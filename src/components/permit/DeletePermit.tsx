  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div className="relative overflow-hidden"
        style={{ background: "linear-gradient(135deg, #7f1d1d, #991b1b, #b91c1c)", padding: "16px 20px" }}>
        ...
        <div className="relative flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-[16px] font-bold text-white leading-none">Delete Permit</h2>
            </div>
          </div>
      ...
      <DialogContent sx={{ p: 3, background: "#fff" }}>
        <div className="rounded-2xl p-4" style={{ background: "#fef2f2", border: "1.5px solid #fecaca" }}>
          <p className="text-[13px] font-bold text-red-800 mb-1">{permit.permit_no}</p>
          <p className="text-[11px] text-red-600/70">{permit.permit_type} — {permit.creator.name}</p>
        </div>
        <div className="mt-3 flex items-start gap-3 rounded-2xl p-3"
          ...
          <p className="text-[12px] leading-relaxed text-orange-800">
            This action is <strong>irreversible</strong>. Once deleted, this permit application and all
            associated records cannot be restored.
          </p>
        </div>
      </DialogContent>
      ...
      <DialogActions sx={{
        ...
        <Button loading={isPending} onClick={handleDelete} sx={{
          background: "linear-gradient(135deg, #991b1b, #b91c1c)", color: "#fff",
          borderRadius: "10px", textTransform: "none", fontSize: "0.8rem",
          fontWeight: 700, px: 2.5, boxShadow: "0 4px 12px rgba(185,28,28,0.35)",
          "&:hover": { background: "linear-gradient(135deg, #b91c1c, #dc2626)", boxShadow: "0 6px 16px rgba(185,28,28,0.45)" },
        }}>
          Yes, Delete Permit
        </Button>
      </DialogActions>
  ...
  return (
    <>
      <IconButton onClick={disclosure.onOpen} size="small"
        sx={{ color: "#991b1b", borderRadius: "8px", "&:hover": { background: "#fee2e2" } }}>
