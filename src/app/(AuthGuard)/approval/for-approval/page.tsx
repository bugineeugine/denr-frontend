...
const EmptyState = () => (
  <div className="flex flex-col items-center justify-center gap-4 py-20">
    ...
    <div className="text-center">
      <p className="text-[15px] font-bold text-slate-700">No pending applications</p>
      <p className="mt-1 text-[13px] text-slate-400">Applications awaiting your action will appear here.</p>
    </div>
  </div>
);
  ...
  const columns = useMemo<MRT_ColumnDef<PermitDataType>[]>(
    () => [
        ...
        Cell: ({ row }) => {
          ...
          return (
            <div className="flex items-center gap-2">
              <Avatar
                sx={{ width: 28, height: 28, fontSize: "0.65rem", fontWeight: 800, background: bg, flexShrink: 0 }}
              >
                {getInitials(name)}
              </Avatar>
      ...
      },
      {
        ...
        header: "",
        size: 60,
        Cell: ({ row }) => <PermitDrawer permit={row.original} />,
      },
    ],
    [],
  );
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbar: ({ table }) => (
      ...
      >
        ...
        <span className="ml-auto text-[11px] font-semibold text-slate-400">{data?.data?.length ?? 0} pending</span>
      </div>
    ),
  });
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      >
        ...
        <div className="relative flex items-center gap-4">
          ...
          >
            <HourglassTopOutlinedIcon sx={{ fontSize: 22, color: "#fff" }} />
          </div>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-emerald-300/70">DENR CENRO</p>
            <h1 className="text-[18px] font-bold leading-tight text-white">For Approval</h1>
            <p className="text-[11px] text-emerald-200/60 mt-0.5">Applications awaiting your action</p>
          </div>
        </div>
      </div>
